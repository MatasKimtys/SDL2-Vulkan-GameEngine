...
void Game::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
...
void Game::recreateSwapChain() {
    int32_t width = 0;
    int32_t height = 0;
    SDL_Vulkan_GetDrawableSize(sdl.getWindow(), &width, &height);
    while (width == 0 || height == 0) {
        SDL_Vulkan_GetDrawableSize(sdl.getWindow(), &width, &height);
        SDL_WaitEvent(nullptr);  // Wait until an event (e.g., resize) occurs
    }
...
void Game::DestroyDebugUtilsMessengerEXT(VkInstance instance, VkDebugUtilsMessengerEXT debugMessenger, const VkAllocationCallbacks* pAllocator) {
...
VkExtent2D Game::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
    if (capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max()) {
        ...
        const auto [width, height] = sdl.getWindowSize();
...
void Game::createSwapChain() {
    ...
    VkSwapchainCreateInfoKHR createInfo{
        ...
        .queueFamilyIndexCount = (indices.graphicsFamily != indices.presentFamily) ? (uint32_t) 2 : 0,
...
void Game::createSurface() {
    if (!SDL_Vulkan_CreateSurface(sdl.getWindow(), instance, &surface)) {
